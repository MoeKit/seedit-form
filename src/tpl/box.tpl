<div id="{{id}}" style="height:100%">
	<div class="personal-info-tips personal-info-sub">{{subTips}}</div>
	<ul class="user-info">
		<li class="clearfix">
			<div class="user-info-text">
				<span class="user-info-name">{{realname}}</span>
				<span class="user-info-phone">{{mobile}}</span>
				<span class="user-info-addr">{{prov}} {{city}} {{ditrict}} {{addr}}</span>
			</div>
		</li>
	</ul>
	<ul class="personal-info-list">{{body}}</ul>
	<div class="personal-info-tips2">{{tips}}</div>
	<button type="button" class="personal-info-btn personal-btn-sub">{{subBtn}}</button>
	<button type="button" class="personal-info-btn personal-btn-close">{{closeBtn}}</button>
</div>